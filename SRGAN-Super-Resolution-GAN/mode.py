...
def test_only(args):
    ...
    dataset = testOnly_data(LR_paths=args.LR_path, in_memory=False, transform=None)
    loader = DataLoader(dataset, batch_size=1, shuffle=False, num_workers=args.num_workers)
    ...
    with torch.no_grad():
        for i, te_data in enumerate(loader):
            ...
            # Extract the base name of the input image and append '_upgraded'
            input_base_name = os.path.basename(dataset.LR_img[i])
            file_name, file_ext = os.path.splitext(input_base_name)
            result_file_name = f'{file_name}_upgraded{file_ext}'

            # Save the result with the new name
            result.save(os.path.join('./result', result_file_name))
