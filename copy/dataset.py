...
class testOnly_data(Dataset):
    def __init__(self, LR_paths, in_memory=True, transform=None):
        ...
        if in_memory:
            # Load all images into memory if in_memory is True
            self.LR_img = [np.array(Image.open(lr)) for lr in self.LR_img]
    ...
    def __getitem__(self, i):
        ...
        if self.in_memory:
            ...
            LR = np.array(Image.open(self.LR_img[i]))
